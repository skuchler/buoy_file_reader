    ...
    def parse(line, check=False):
        ...
        if talker_match:
            ...
            if cls:
                ...
                free_match = NMEASentence.free_re.match(sentence_type)
                if free_match:
                    sentence = free_match.group('free_type')
                    cls = FreeSentence.sentence_types.get(sentence)
                    if not cls:
                        # TODO instantiate base type instead of fail
                        raise SentenceTypeError(
                            'Unknown sentence type %s' % sentence_type, line)
                    return cls(sentence, data)
...
class FreeSentence(NMEASentence):
    sentence_types = {}
